...
class OpRollNumber(models.Model):
    ...
    state = fields.Selection(
        [
            ("active", u"Asistiendo"),
            ("inactive", u"Congelado"),
            ("done", u"Incorporado"),
            ("gone", u"Retirado"),
            ("relocated", u"Cambio de escuela"),
            ("bochanged", u"Cambio de sucursal"),
            ("bchange", u"Cambio de beneficiario"),
            ("inactive", u"No asiste"),
        ],
        default="active",
        string=u"Estado",
    )
    ...
    previous_roll_number_id = fields.Many2one(
        "op.roll.number", string=u"Matrícula anterior"
    )
    ...
    def open_relocate_wizard(self):
        wizard_form = self.env.ref(
            "oeducat_roll_state.view_wizard_relocate_student_form", False
        )
